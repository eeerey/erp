...
// 1. Definisikan opsi kolom khusus untuk RAK
const rakColumnOptions = [
  { name: "Kode Rak", value: "KODE_RAK", dataKey: "KODE_RAK" },
  { name: "Nama Rak", value: "NAMA_RAK", dataKey: "NAMA_RAK" },
  { name: "Kode Gudang", value: "KODE_GUDANG", dataKey: "KODE_GUDANG" },
  { name: "Nama Gudang", value: "NAMA_GUDANG", dataKey: "NAMA_GUDANG" }, // Relasi
...
export default function AdjustPrintLaporan({
  ...
  setFileName,
  setJsPdfPreviewOpen,
  judulLaporan = "LAPORAN DATA MASTER RAK", // Default Judul Rak
  namaPenandatangan,
  ...
  const [config, setConfig] = useState({
    ...
    selectedColumns: ["KODE_RAK", "NAMA_RAK", "KODE_GUDANG", "NAMA_GUDANG"], 
  });
  ...
  const generatePDF = () => {
    ...
    doc.text("Warehouse Management System (WMS)", pageWidth / 2, currentY, { align: "center" });
    ...
    const tableColumn = config.selectedColumns.map((colValue) => {
      const colConfig = rakColumnOptions.find((c) => c.value === colValue);
    ...
    const tableRows = dataToPrint.map((item) => {
      ...
      config.selectedColumns.forEach((colKey) => {
        // Logika untuk mengambil data dari relasi (misal item.gudang.NAMA_GUDANG)
        let val = item[colKey] || "-";
        
        // Jika data rak Anda dikirim dari API dengan JOIN gudang:
        if (colKey === "NAMA_GUDANG" && !item[colKey]) {
            val = item.gudang?.NAMA_GUDANG || "-";
        }
  ...
  return (
    <Dialog 
        ...
        header={<div className="flex align-items-center gap-2"><i className="pi pi-print"></i> Cetak Laporan Rak</div>} 
        style={{ width: "450px" }} 
        modal 
        footer={footer}
    >
      <div className="grid p-fluid">
        <div className="col-12 field">
          <label className="font-bold mb-2 block">Pilih Kolom Rak</label>
          <MultiSelect 
            value={config.selectedColumns} 
            options={rakColumnOptions} 
