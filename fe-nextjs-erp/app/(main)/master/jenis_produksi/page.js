...
export default function ProduksiPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`${API_URL}/produksi`); // 🔥 UBAH
  ...
  const handleSearch = (keyword) => {
    ...
    const filtered = originalData.filter(v => 
      ...
      v.BARANG_KODE?.toLowerCase().includes(keyword.toLowerCase()) // 🔥 TAMBAHAN
    );
  ...
  const handleSubmit = async (payload) => {
    try {
      let res;
      if (selectedData) {
        res = await axios.put(`${API_URL}/produksi/${selectedData.ID}`, payload);
      } else {
        res = await axios.post(`${API_URL}/produksi`, payload);
      }
  ...
  const handleDelete = (rowData) => {
    confirmDialog({
      ...
      accept: async () => {
        try {
          const res = await axios.delete(`${API_URL}/produksi/${rowData.ID}`);
          if (res.data.status === "00") {
  ...
  const columns = [
    ...
    { field: "BARANG_KODE", header: "Kode Barang", sortable: true }, // 🔥 TAMBAHAN
