...
export default function MasterHariPage() {
    ...
    useEffect(() => {
        // Ambil token dari key TOKEN (kapital) atau token (kecil)
        const t = localStorage.getItem('TOKEN') || localStorage.getItem('token');
    ...
    const statusBodyTemplate = (rowData) => <Tag value={rowData.STATUS} severity={rowData.STATUS === 'Aktif' ? 'success' : 'danger'} />;

    const hariKerjaBodyTemplate = (rowData) => <span>{rowData.IS_HARI_KERJA ? 'Ya' : 'Tidak'}</span>;
    ...
    const actionBodyTemplate = (rowData) => (
        <div className="flex gap-2 justify-content-center">
            ...
            />
            <Button icon="pi pi-trash" className="p-button-rounded p-button-danger p-button-sm" onClick={() => handleDelete(rowData)} />
        </div>
    );
