...
export async function up(knex) {
  ...
  await knex.schema.createTable("master_shift", (table) => {
    ...
    table
      .time("JAM_MASUK")
      .notNullable()
      .comment("Jam masuk standar, misal 07:00:00");
    table
      .time("JAM_KELUAR")
      .notNullable()
      .comment("Jam keluar standar, misal 15:00:00");
    table
  ...
  // 3. Ubah SHIFT di master_karyawan dari ENUM → VARCHAR
  await knex.schema.alterTable("master_karyawan", (table) => {
...
export async function down(knex) {
  // Langkah 1: Hapus Foreign Key constraint terlebih dahulu secara terpisah
  await knex.schema.alterTable("master_karyawan", (table) => {
    table.dropForeign(["SHIFT"]);
  });

  // Langkah 2: Kembalikan tipe kolom SHIFT ke ENUM
  await knex.schema.alterTable("master_karyawan", (table) => {
    table.enu("SHIFT", ["Pagi", "Siang", "Malam"]).nullable().alter();
  });

  // Langkah 3: Baru drop tabel master_shift
