...
export const getAllGudang = async (req, res) => {
  try {
    ...
    const data = await MasterGudangModel.getAllGudang(companyId);
  ...
  try {
    ...
    if (!KODE_GUDANG || !NAMA_GUDANG) {
      return res.status(400).json({
        status: "01",
        message: "KODE_GUDANG dan NAMA_GUDANG wajib diisi",
      });
    }

    const companyId = req.user.company_id;
    // Cek duplikat KODE_GUDANG
    const existing = await MasterGudangModel.getGudangByKode(
      KODE_GUDANG,
      companyId,
    );
    if (existing) {
      return res.status(409).json({
        status: "02",
        message: `Gudang dengan kode ${KODE_GUDANG} sudah ada`,
      });
    }

    console.log("COMPANY ID =", companyId);

    const newGudang = await MasterGudangModel.createGudang({
      KODE_GUDANG,
      NAMA_GUDANG,
      ALAMAT,
      STATUS,
...
export const updateGudang = async (req, res) => {
  try {
    ...
    if (!id || id === "undefined") {
      return res
        .status(400)
        .json({ status: "01", message: "ID Gudang tidak valid" });
    }

    const existing = await MasterGudangModel.getGudangById(id, companyId);
    if (!existing) {
      return res
        .status(404)
        .json({ status: "04", message: "Gudang tidak ditemukan" });
    }

    // Filter body untuk memastikan tidak mencoba update primary key (ID_GUDANG)
    const { ID_GUDANG, ...updateData } = req.body;

    const updated = await MasterGudangModel.updateGudang(
      id,
      companyId,
      updateData,
    );
...
export const deleteGudang = async (req, res) => {
  try {
    ...
    await MasterGudangModel.deleteGudang(id, companyId);
    return res
      .status(200)
      .json({ status: "00", message: "Gudang berhasil dihapus" });
  } catch (err) {
    return res.status(500).json({
      status: "99",
